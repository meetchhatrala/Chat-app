...
function MessageDetail() {
  ...
  const [user, setUser] = useState([]);
  const [profile, setProfile] = useState([]);
  const [newMessage, setNewMessage] = useState({ message: "", file: null });
  ...
  const handleFileChange = (event) => {
    ...
    setNewMessage(prevState => ({ ...prevState, [name]: files[0] }));
  };
  ...
  const SendMessage = () => {
    ...
    axios.post(`${baseURL}/send-messages/`, formData)
      .then((res) => {
        document.getElementById("text-input").value = "";
        document.getElementById("file-input").value = "";
        setNewMessage({ message: "", file: null });
      })
  ...
  const handleSearchChange = (event) => {
    const { name, value } = event.target;
    setNewSearch(prevState => ({ ...prevState, [name]: value }));
  };
              ...
              <div className="col-12 col-lg-5 col-xl-3 border-right">
                ...
                {messages.map((message) =>
                  ...
                  >
                    ...
                    <div className="d-flex align-items-start">
                      <img
                        src={message.sender.id !== user_id ? message.sender_profile.image : message.reciever_profile.image}
                        className="rounded-circle mr-1"
                        alt="Profile"
                        width={40}
                        height={40}
